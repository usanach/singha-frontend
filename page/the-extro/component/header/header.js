      ...
      <div class="bg [background-image:radial-gradient(circle,_rgba(255,255,255,1)_0%,_rgba(255,255,255,1)_100%)] py-2 relative z-50 flex transition-all">
        <div class="absolute inset-0  flex items-center justify-center">
            <div class="text-center">
                <p class="uppercase text-[#1a2f4d]  text-[14px]">Residential</p>
            </div>
        </div>
        <div class="flex container mx-auto justify-between relative my-auto">
          <div>
            <a :href="'/' + language">
              <img class="w-[110px] md:block hidden" src="/assets/image/page-smyth-home/header/logo.png" />
          ...
          <div class="my-auto flex gap-3  relative ml-auto">
            <div>
                <button type="button" class="lg:flex hidden h-full" @click="toggleSubModal">
                    <div class="my-auto">
                        <p class="uppercase text-[#1a2f4d] leading-tight text-[12px]" :class="singhaFonts">
            ...
            <div class="relative px-2" ref="langRef">
                <button type="button" class="lg:flex hidden h-full" @click="toggleLanguage">
                    <div class="my-auto flex gap-2">
                        <svg class="w-[15px] my-auto" width="20" height="20" viewBox="0 0 20 20" fill="#1a2f4d" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <p class="uppercase text-[#1a2f4d] text-[12px] my-auto">{{language}}</p>
                        <svg class="my-auto transition-all duration-500 w-[15px]" width="20" height="20" viewBox="0 0 20 20" fill="#1a2f4d" xmlns="http://www.w3.org/2000/svg">
                ...
                <div v-show="isSelectLanguage" class="absolute top-100 mt-2 left-0 w-full h-fit gap-2 bg-[#1A2F4D] p-2 z-[60] text-center space-y-2">
                    <div class="uppercase text-white text-center hover:bg-[#162842] text-[12px] pt-1 leading-none flex">
            ...
            <div class="flex">
                <button type="button" class="my-auto" @click="toggleMainModal">
                    <div  class="w-[15px] h-[15px] relative">
                        ...
                        >
                        <svg v-if="!isMainModalOpen" class="absolute inset-0 w-full h-full" width="20" height="20" viewBox="0 0 20 20" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
        ...
        <div v-show="isSubModalOpen" class="lg:block hidden absolute left-0 w-full">
          ...
          >
            ...
            >
                ...
                <p class="px-4 my-auto text-[16px] text-white capitalize font-normal border border-[5px] border-r-0 border-t-0 border-b-0 border-[#948668]">
                    {{ headerData?.swipeSub?.title[language] }}
                </p>
                                                ...
                                                >
                                                    ...
                                                    <div class="text-left">
                                                        <a 
                                                            ...
                                                            @click.prevent="mobileReady[idx] && selectCard(slide)"
                                                        >
                                                            <small class="text-[12px] uppercase font-thin leading-tight">
                                                            {{ slide.type[language] }}
                                                            </small>
                                                            <p class="text-[16px] leading-tight">
                                                            {{ slide.title[language] }}
                ...
                <div class="flex gap-10 relative flex-col">
                    <div class="flex mt-auto justify-between">
                        ...
                        <div class="flex mt-auto mb-1 gap-2">
                            <div>
                                <a :href="'/en'+path" class="text-white uppercase">
                                    en
                                </a>
                            </div>
    ...
    setup() {
        ...
        const init = () => {
            ...
            watch(hoveredIdx, async () => {
                await nextTick(); // ให้ DOM render สไลด์ใหม่เสร็จก่อน
                if (swiperMain && swiperMain.update) {
                    swiperMain.update();
                    swiperMain.slideTo(0); // กลับไปหน้าแรกของเมนู
                }
            });
        ...
        onMounted(async () => {
            ...
            const res = await axios.get('/page/the-extro/component/header/header.json');
            headerData.value = {
                data: res.data,
                swipeSub: {
                    title: {
