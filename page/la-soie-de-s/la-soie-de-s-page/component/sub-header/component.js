const SubHeaderComponent = defineComponent({
            ...
            <div class="w-full flex justify-center my-auto gap-5">
              <div v-for="(link, index) in links" :key="link.id">
                <a :href="link.url[language]" @click="setActive(index)" :data-header-click="link.url['en']" class="cursor-pointer">
                  <p class="uppercase text-center transition-colors" :class="activeIndex === index ? 'font-bold' : 'font-normal'" v-html="link.name[language]">
                  </p>
                </a>
              </div>
            </div>
  ...
  setup() {
    ...
    const logo = ref('/assets\/image\/page-la-soie-de-s\/logo.png');
    ...
    const updateSubHeaderStyle = (progress) => {
      if (subHeader.value && logoRef.value) {
        ...
        if (progress > 0) {
          ...
          logoRef.value.src = '/assets\/image\/page-la-soie-de-s\/logo lasoides.png';
          ...
          logoRef.value.src = '/assets\/image\/page-la-soie-de-s\/logo.png';
