...
const FormRegisterComponent = defineComponent({
        ...
        <section class="form-register onview font-['IBM_Plex_Sans_Thai']" id="register" data-section="lead_register">
            ...
            <div class="fixed inset-0 bg-black bg-opacity-75 z-[9999]" :class="[isSuccess ? 'block':'hidden']">
                <div class="p-5 rounded-lg h-full flex">
                    <div class="m-auto">
                        <img aria-hidden="true" src="/assets\/image\/page-siraninn\/form-register\/srn_thnkyou_bn-popup.jpg" class="lg:block hidden" />
                        <img aria-hidden="true" src="/assets\/image\/page-siraninn\/form-register\/srn_thnkyou_bn-popup-m.jpg" class="lg:hidden" />
                    </div>
                </div>
    ...
    setup() {
        ...
        const BgImage= ref("/assets\/image\/page-siraninn\/form-register\/_DSC7988.png")
        const mobileBgImage= ref("/assets\/image\/page-siraninn\/form-register\/_DSC7988.png")
        const regBgImage= ref("/assets\/image\/page-siraninn\/form-register\/Group-2511.png")
        const form_text = ref({
            title:{
        ...
        const validateForm = async () => {
            ...
            if (Object.values(errors.value).every(error => !error)) {
                ...
                try {
                    ...
                    object.token = token;
                    await axios.post(`https://residential2.singhaestate.co.th/th/lead/siraninn/droplead.php`, object);
        ...
        const fetchBudgets = async () => {
            try {
                const response = await axios.get('/page\/siraninn\/siraninn-page\/data\/budget.json');
