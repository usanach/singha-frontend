...
const FormRegisterComponent = defineComponent({
  ...
  template: `
    ...
    <section
      v-if="isFormEnabled"
      class="form-register onview font-['IBM_Plex_SANS_Thai']"
      id="register"
    ...
    >
      <div class="relative lg:h-[900px]">
        <div class="flex lg:flex-row flex-col h-full w-full">
          ...
          <!-- RIGHT: Form BG -->
          <div
            class="w-full h-full bg-cover bg-center flex relative backdrop-filter backdrop-brightness- opacity-100 backdrop-blur-md"
            :style="formImage
                    ...
                    <div class="flex flex-col gap-10">
                      ...
                      <div class="flex gap-8 lg:flex-row flex-col">
                        <div class="lg:w-1/2 w-full relative">
                          <label for="province"
                            class="text-white w-full absolute top-0 left-0 w-full h-full cursor-pointer">
                            {{ selectedProvince == null ? form_text.province[language] : '' }}
                          </label>
                        ...
                        <div class="lg:w-1/2 w-full relative">
                          <label for="district"
                            class="text-white w-full absolute top-0 left-0 w-full h-full cursor-pointer">
                            {{ selectedDistrict == null ? form_text.district[language] : '' }}
                          </label>
                          <select name="district" id="district" v-model="selectedDistrict"
                            ...
                            <option v-if="selectedProvince != null"
                              v-for="district in filteredDistricts" :key="district.id" :value="district.id" class="text-black">
                      ...
                      <div class="flex gap-8 lg:flex-row flex-col">
                        <div class="w-full">
                          <div class="relative">
                            <label for="budget"
                              class="text-white w-full absolute top-0 left-0 w-full h-full cursor-pointer">
                              {{ selectedBudget !== null ? '' : form_text.budgets[language] }}
                            </label>
                            <select name="budget" id="budget" v-model="selectedBudget"
                              class="text-white bg-transparent border border-b-1 border-l-0 border-t-0 border-r-0 w-full relative cursor-pointer">
                              <option v-for="budget in budgets" :key="budget.id" :value="budget.title[language]" class="text-black">
                          ...
                          <div class="flex items-center space-x-2">
                            <div class="relative mb-auto">
                              <input
                                type="checkbox"
                                name="consents"
                                :data-value="form.consents"
                                id="custom-checkbox"
                                v-model="form.consents"
                                class="hidden peer"
                              >
                              ...
                              >
                                <img src="/assets/icon/checked-white.svg" alt="" class="w-[10px] h-[10px]">
                              </label>
                            </div>
                      ...
                      <div class="mx-auto">
                        ...
                        >
                          <div class="loaded">
                            <p class="text-nowrap font-normal text-white">{{ form_text.submit[language] }}</p>
                          </div>
                          <div class="loading hidden">
                            <svg width="25" height="25" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" fill="none">
                              <circle cx="50" cy="50" r="40" stroke="#fff" stroke-width="10" opacity="0.2" />
                              <path d="M90 50a40 40 0 0 1-40 40" stroke="#fff" stroke-width="10" stroke-linecap="round">
      ...
      <div class="fixed inset-0 bg-black bg-opacity-75 z-[9999]" :class="[isSuccess ? 'block':'hidden']">
        <div class="p-5 rounded-lg h-full flex">
          <div class="m-auto">
            <img :src="thankYouDesktop || ''" class="lg:block hidden" />
            <img :src="thankYouMobile || ''" class="lg:hidden" />
          </div>
        </div>
  ...
  setup() {
    ...
    // ✅ form_disabled = true => แสดง section
    ...
    const form_text = ref({
      ...
      submit: { en: "Submit", th: "ลงทะเบียน" },
      fname:  { en: "First Name*", th: "ชื่อ*" },
      sname:  { en: "Surname*", th: "นามสกุล*" },
      email:  { en: "Email*", th: "อีเมล*" },
      tel:    { en: "Mobile*", th: "เบอร์โทรศัพท์*" },
      budgets:{ en: "Budget", th: "งบประมาณ" },
      province:{ en: "Province*", th: "จังหวัด*" },
      district:{ en: "Districts*", th: "อำเภอ*" },
      consents: {
    ...
    const validateForm = async () => {
      ...
      if (!Object.values(errors.value).every(e => !e)) return;
    ...
    const filterDistricts = () => {
      if (!selectedProvince.value) {
        ...
        filteredDistricts.value = districts.value.filter(d => d.province_id === selectedProvince.value);
      }
    };
    ...
    const fetchBudgets = async () => {
      try {
        ...
        const res = await getProjectBudget(projectId.value); // ✅ api.js
    ...
    // ✅ ใช้ api.js getProjectForm และ form_disabled เป็น boolean:
    // form_disabled: true => เปิดการมองเห็น section
    const fetchFormConfig = async () => {
      try {
        if (!projectIDs) {
          console.warn('No projectIDs for form');
        ...
        const res = await getProjectForm(projectId.value); // ✅ api.js
        const rows = Array.isArray(res?.data?.data) ? res.data.data : [];
        ...
        // ✅ true = เปิดให้แสดง
        isFormEnabled.value = !!row.form_disabled;

        if (!isFormEnabled.value) return;

        // title
        if (row.form_title) {
          if (row.form_title.th != null) form_text.value.title.th = row.form_title.th;
          if (row.form_title.en != null) form_text.value.title.en = row.form_title.en;
        }

        // images
        sideImage.value = row.side_image ? buildFormImagePath(row.side_image) : '';
        formImage.value = row.form_image ? buildFormImagePath(row.form_image) : '';

        // thank you images (ตามภาษา)
        ...
        thankYouDesktop.value = row[tyDesktopKey] ? buildThankYouImagePath(row[tyDesktopKey]) : '';
        thankYouMobile.value  = row[tyMobileKey]  ? buildThankYouImagePath(row[tyMobileKey])  : '';
      } catch (error) {
    ...
    onMounted(async () => {
      ...
      await fetchFormConfig(); // ✅ ต้องมาก่อน budgets เพราะ set projectId + toggle
